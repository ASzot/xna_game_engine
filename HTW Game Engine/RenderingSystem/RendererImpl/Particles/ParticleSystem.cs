...
namespace RenderingSystem.RendererImpl
{
    public class ParticleSystem
    {
        ...
        private EffectParameter _effectViewportScaleParameter;
        private EffectParameter _effectTimeParameter;
        private EffectParameter _effectAlphaReference;
        ...
        public ParticleSystem(GraphicsDevice device, ContentManager content, string settingsName)
        {
            ...
            this._settings = content.Load<ParticleSettings>(settingsName);
            Initialize(content);
            LoadParticleEffect(content);
        }

        public ParticleSystem(GraphicsDevice device, ContentManager content, ParticleSettings particleSettings)
        {
            this.settingsName = "";
            this._settings = particleSettings;
            this._graphicsDevice = device;
            Initialize(content);
            LoadParticleEffect(content);
        }
        ...
        public void Restart()
        {
            _currentTime = _totalTime = _accumTime = 0;
            _drawCounter = 0;
            _canEmmit = true;
            _firstActiveParticle = _firstNewParticle = _firstFreeParticle = _firstRetiredParticle = 0;
        }
        ...
        protected void Initialize(ContentManager content)
        {
            ...
            this._particleEffect = content.Load<Effect>("particles/ParticleEffect");
            ...
            _vertexBuffer = new DynamicVertexBuffer(_graphicsDevice, ParticleVertex.VertexDeclaration,
            ...
            float maxVel = Math.Max(Math.Max(_settings.MaxHorizontalVelocity, _settings.MinHorizontalVelocity),
                                    Math.Max(_settings.MaxVerticalVelocity, _settings.MinVerticalVelocity));
            float minVel = Math.Min(Math.Min(_settings.MaxHorizontalVelocity, _settings.MinHorizontalVelocity),
                                    Math.Min(_settings.MaxVerticalVelocity, _settings.MinVerticalVelocity));
            Vector3 max = Vector3.One* (_settings.MaxEndSize/2 + (float)(Math.Abs(_settings.Duration.TotalSeconds*maxVel)));
            Vector3 min = -Vector3.One * (_settings.MaxEndSize/2 + (float)(Math.Abs(_settings.Duration.TotalSeconds * minVel)));
        ...
        {
            return _firstActiveParticle != _firstFreeParticle || (_totalTime < Settings.Duration.TotalSeconds || Settings.Duration.TotalSeconds < 0);
        }
        ...
        void LoadParticleEffect(ContentManager content)
        {
            ...
            _particleEffect = _particleEffect.Clone();
            ...
            Texture2D texture = content.Load<Texture2D>(_settings.TextureName);
        ...
        public void Update(float deltaTime)
        {
            ...
            if ( _settings.EmissionRate > 0 && _canEmmit)
            {
                _accumTime += deltaTime;
                float emissionTime =1.0f/Settings.EmissionRate;
                while (_accumTime > emissionTime)
                {
                    AddParticle();
                    _accumTime -= emissionTime;
                }
            }
        ...
        public void Draw()
        {
            ...
            if (_firstActiveParticle != _firstFreeParticle)
            {
                _graphicsDevice.BlendState = Settings.BlendState;

                // Set an effect parameter describing the viewport size. This is
                // needed to convert particle sizes into screen space point sizes.
                _effectViewportScaleParameter.SetValue(new Vector2(0.5f / _graphicsDevice.Viewport.AspectRatio, -0.5f));
                ...
                _particleEffect.CurrentTechnique = _particleEffect.Techniques[0];
        ...
        public void AddParticle()
        {
            ...
            double horizontalAngle = _random.NextDouble() * MathHelper.TwoPi;
            ...
            Color randomValues = new Color((byte)_random.Next(255),
