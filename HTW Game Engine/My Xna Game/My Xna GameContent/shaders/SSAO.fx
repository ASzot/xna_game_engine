...
[branch]
	...
	if (depth < 1)
	{
		occ  = tex2D(ssaoSampler, input.TexCoord.zw).r;
		...
		{
			float2 uvOcc    = input.TexCoord.zw + BlurDirection* (i);
			half newOcc = tex2D(ssaoSampler, uvOcc).r;
			float2 uvScreen = input.TexCoord.xy + BlurDirection* (i);
...
{
    float4 Position : POSITION0;
	float2 TexCoordSSAO : TEXCOORD0;
};

VertexShaderOutputMix VertexShaderFinalMix(VertexShaderInput input)
{
    VertexShaderOutputMix output = (VertexShaderOutputMix)0;
	
	output.Position = input.Position;
	output.TexCoordSSAO.xy = input.TexCoord + 0.5f/SSAORes;
	return output;
}

float4 PixelShaderFinalMix(VertexShaderOutputMix input) : COLOR0
{
	half ssao = tex2D(ssaoSampler, input.TexCoordSSAO).r*SSAOIntensity;
	return ssao;
}
